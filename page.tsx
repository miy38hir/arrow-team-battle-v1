...
import { drawRank, gachaNames, gachaPoints, type GachaType, type Rank } from '@/lib/rules';
...
function rankClass(rank?: Rank) {
  return rank ? `rank-${rank.toLowerCase()}` : '';
}

function playGachaSound(rank: Rank) {
  try {
    const AudioCtx = window.AudioContext || (window as unknown as { webkitAudioContext: typeof AudioContext }).webkitAudioContext;
    const ctx = new AudioCtx();
    const notes = rank === 'S' ? [523, 659, 784, 1046] : rank === 'A' ? [440, 554, 659] : [330, 392, 494];
    notes.forEach((freq, index) => {
      ...
      gain.gain.exponentialRampToValueAtTime(0.13, ctx.currentTime + index * 0.13 + 0.03);
      gain.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + index * 0.13 + 0.22);
      osc.connect(gain);
      gain.connect(ctx.destination);
      osc.start(ctx.currentTime + index * 0.13);
      osc.stop(ctx.currentTime + index * 0.13 + 0.24);
    });
...
export default function CastPage() {
  ...
  const [animationGachaName, setAnimationGachaName] = useState('');
  ...
  async function draw(ticketId: string) {
    ...
    if (!ticket || ticket.used || isAnimating) return;

    setDrawingTicketId(ticketId);
    setAnimationGachaName(gachaNames[ticket.gachaType]);
    setLastResult(null);
    setIsAnimating(true);
    ...
    setLastResult({
      rank,
      point,
      name: gachaNames[ticket.gachaType],
      castName: ticket.castName,
      teamName: ticket.teamName,
    });
    playGachaSound(rank);
  ...
  return (
    <main className={`container ${lastResult ? rankClass(lastResult.rank) : ''}`}>
      ...
      <div className="grid">
        ...
        <section className="card col-4 gachaResultCard">
          <h2>ガチャ結果</h2>
          <div className={`gachaBox ${isAnimating ? 'is-drawing' : ''} ${lastResult ? rankClass(lastResult.rank) : ''}`}>
            {isAnimating ? (
              <>
                <div className="gachaStageLabel">{animationGachaName}</div>
                <div className="gachaMachine" aria-hidden="true">
                  <div className="gachaGlow" />
                  <div className="gachaOrb orb1" />
                  <div className="gachaOrb orb2" />
                  <div className="gachaOrb orb3" />
                  <div className="gachaCapsule" />
                </div>
                <div className="drumroll">抽選中...</div>
                <div className="loadingDots"><span /> <span /> <span /></div>
              </>
            ) : lastResult ? (
              <>
                {(lastResult.rank === 'S' || lastResult.rank === 'A') && (
                  <div className="confetti" aria-hidden="true">
                    {Array.from({ length: 28 }).map((_, i) => <i key={i} style={{ '--i': i } as React.CSSProperties} />)}
                  </div>
                )}
                <div className="resultBurst" aria-hidden="true" />
                <div className="badge resultBadge">{lastResult.name}</div>
                <div className="rankSub">{rankLabels[lastResult.rank]}</div>
                <div className="gachaRank">{lastResult.rank}賞</div>
                <div className="point resultPoint">+{lastResult.point}pt</div>
                <p className="resultText">{lastResult.castName} が {lastResult.teamName} に貢献！</p>
              </>
            ) : (
        ...
        <section className="card col-6">
          ...
          <div className="row gachaButtonRow">
            {myTickets.map(t => (
              <button key={t.id} className="button primary gachaButton" disabled={drawingTicketId === t.id || isAnimating} onClick={() => draw(t.id)}>
                <span className="buttonShine" />
                {drawingTicketId === t.id ? '抽選中...' : `${gachaNames[t.gachaType]}を回す`}
              </button>
            ))}
          </div>
        </section>
